...
def plot_acc_loss_f1_auc(model_history, folder_path):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(os.path.join(folder_path, 'loss_t2.png'))
    plt.close()
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(os.path.join(folder_path, 'accuracy_t2.png'))
    plt.close()
...
#     plt.close()
    ...
    plt.ylabel('F1 Score')
    plt.legend()
    plt.savefig(os.path.join(folder_path, 'f1_score_t2.png'))
    plt.close()
...
history_effnet = model_train(model_name = EfficientNetB0(weights='imagenet', include_top=False, input_shape=(224,224,3)), save_name = "effnet_t2", image_size = 224, learning_rate = 0.001, dropout=0.7, optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001), dense_0_units=96, dense_1_units=None, batch_size=32)
#history_densenet_weights = model_train(model_name = tf.keras.applications.densenet.DenseNet121(include_top=False, weights='imagenet', input_shape=(224,224,3), classes=2), save_name = "densenet_t2", image_size = 224, dropout=0.5, optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001), dense_0_units=32, dense_1_units=None, batch_size=32)
