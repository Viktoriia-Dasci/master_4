...
def f1_score(y_true, y_pred, weight_class1=2):
    ...
    # Adjusted precision and recall with weight for class 1
    precision = tp / (tp + (weight_class1 * fp) + tf.keras.backend.epsilon())
    recall = tp / (tp + (weight_class1 * fn) + tf.keras.backend.epsilon())
