...
#load data
HGG_list_train = load_from_dir(f'/content/drive/MyDrive/Split_data/stacked_mri_new/train/HGG_{modality}')
LGG_list_train = load_from_dir(f'/content/drive/MyDrive/Split_data/stacked_mri_new/train/LGG_{modality}')
HGG_list_val = load_from_dir(f'/content/drive/MyDrive/Split_data/stacked_mri_new/val/HGG_{modality}')
LGG_list_val = load_from_dir(f'/content/drive/MyDrive/Split_data/stacked_mri_new/val/LGG_{modality}')
HGG_list_test = load_from_dir(f'/content/drive/MyDrive/Split_data/stacked_mri_new/test/HGG_{modality}')
LGG_list_test = load_from_dir(f'/content/drive/MyDrive/Split_data/stacked_mri_new/test/LGG_{modality}')


#preprocessing data
HGG_list_new_train = preprocess_stack(HGG_list_train)
LGG_list_new_train = preprocess_stack(LGG_list_train)

HGG_list_new_val = preprocess_stack(HGG_list_val)
LGG_list_new_val = preprocess_stack(LGG_list_val)

HGG_list_new_test = preprocess_stack(HGG_list_test)
LGG_list_new_test = preprocess_stack(LGG_list_test)
...
# history_inception_weights = model_train(model_name = tf.keras.applications.inception_v3.InceptionV3(include_top=False, weights='imagenet', input_shape=(224,224,3), classes=2), save_name = "inception_stacked", image_size = 224, dropout=0.5, optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001), dense_0_units=16, dense_1_units=128, batch_size=16)
# history_effnet = model_train(model_name = EfficientNetB0(weights='imagenet', include_top=False, input_shape=(224,224,3)), save_name = "effnet_stacked", image_size = 224, dropout=0.7, optimizer=tf.keras.optimizers.Adam(learning_rate=0.001), dense_0_units=48, dense_1_units=None, batch_size=16)
