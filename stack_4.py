...
def plot_acc_loss_f1_auc(model_history, folder_path):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(os.path.join(folder_path, 'loss_stack_4.png'))
    plt.close()
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(os.path.join(folder_path, 'accuracy_stack_4.png'))
    plt.close()
...
#     plt.close()
    ...
    plt.ylabel('F1 Score')
    plt.legend()
    plt.savefig(os.path.join(folder_path, 'f1_score_stack_4.png'))
    plt.close()
...
HGG_list_train = load_from_dir('/home/viktoriia.trokhova/Stacked_4/train/HGG_stack')
LGG_list_train = load_from_dir('/home/viktoriia.trokhova/Stacked_4/train/LGG_stack')
...
new_conv1 = Conv2D(64, (7, 7), strides=(2, 2), padding='valid', activation='relu')(input_layer)
...
history_inception_weights = model_train(model_name = tf.keras.applications.inception_v3.InceptionV3(include_top=False, weights='imagenet', input_shape=(224, 224, 4), classes=2), image_size = 224, learning_rate = 0.1, dropout=0.7)
